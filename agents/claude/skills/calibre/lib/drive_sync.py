...
class DriveSync:
    ...
    def pull_db(self, force=False, progress=None):
        ...
        emit("metadata_db_search_start", local_path=self.db_local)
        files = client.search_root("metadata.db", max_results=5)
        db_file = next(
        ...
        if not force and os.path.exists(self.db_local) and os.path.exists(mtime_cache):
            ...
            if cached_mtime == drive_mtime:
                emit("metadata_db_up_to_date", local_path=self.db_local, modified_time=drive_mtime)
                return False  # already current

        # Download
        tmp_path = self.db_local + ".tmp"
        emit("metadata_db_download_start", local_path=self.db_local, tmp_path=tmp_path, size_bytes=size_bytes)
        client.download_file(
            db_file["id"],
            tmp_path,
            progress=lambda **fields: emit("metadata_db_download_progress", **fields),
        )
        os.replace(tmp_path, self.db_local)
        local_size = os.path.getsize(self.db_local) if os.path.exists(self.db_local) else None
        emit("metadata_db_download_done", local_path=self.db_local, size_bytes=local_size)
