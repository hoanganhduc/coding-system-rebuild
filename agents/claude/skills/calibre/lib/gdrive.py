...
class GDriveClient:
    ...
    def search_root(self, query, max_results=10):
        """Search for files by name directly under the configured root folder."""
        q = f"'{self.folder_id}' in parents and name contains '{_escape(query)}' and trashed = false"
        resp = self.service.files().list(
            q=q,
            fields="files(id, name, mimeType, webViewLink, modifiedTime, size)",
            pageSize=max_results,
    ...
    def download_file(self, file_id, local_path, progress=None):
