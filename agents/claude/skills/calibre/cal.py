...
def _ensure_db(config, sync=False, progress=None):
    ...
    if not os.path.exists(db_path) or sync:
        ds = DriveSync(config)
        if progress:
            progress("metadata_db_lock_wait", local_path=db_path, force=sync)
        with ds.db_lock():
            if progress:
                progress("metadata_db_lock_acquired", local_path=db_path, force=sync)
            downloaded = ds.pull_db(force=sync, progress=progress)
        if progress:
            progress("metadata_db_ready", local_path=db_path, downloaded=downloaded)
...
def cmd_sync(args):
    ...
    if progress:
        progress("sync_start", force=args.force, local_path=config["db_local_path"])

    if progress:
        progress("metadata_db_lock_wait", local_path=config["db_local_path"], force=args.force)
    with ds.db_lock():
        if progress:
            progress("metadata_db_lock_acquired", local_path=config["db_local_path"], force=args.force)
        downloaded = ds.pull_db(force=args.force, progress=progress)
    if progress:
        progress("metadata_db_ready", local_path=config["db_local_path"], downloaded=downloaded)

    if not downloaded and not args.force:
        if progress:
            progress("cache_load_start", cache_path=config["cache_path"])
        items, age_hours = load_cache(config["cache_path"])
        if progress:
            progress("sync_done", downloaded=False, cached_books=len(items))
    ...
    if progress:
        progress("cache_rebuild_start", db_path=config["db_local_path"])
    with CalibreDB(config["db_local_path"]) as db:
    ...
    save_cache(config["cache_path"], items)
    if progress:
        progress("cache_rebuild_done", books=count, cache_path=config["cache_path"])
        progress("sync_done", downloaded=True, books=count)
...
def build_parser():
    ...
    psy.add_argument("--progress", action="store_true", help="Emit metadata.db sync progress as JSON lines to stderr")
