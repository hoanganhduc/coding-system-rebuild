...
class BrokerConfig:
    ...
    def state_root(self, workspace_root: Path) -> Path:
        root = Path(self.broker_state_root).expanduser()
        return root if root.is_absolute() else workspace_root / root


def workspace_root() -> Path:
    for key in (
        "CLAUDE_RESEARCH_COMPUTE_WORKSPACE",
        "CLAUDE_RUNTIME_WORKSPACE",
        "CODEX_RUNTIME_WORKSPACE",
        "OPENCLAW_WORKSPACE",
    ):
...
def caller_cwd() -> Path:
    value = (
        os.environ.get("CLAUDE_CALLER_CWD")
...
def load_config(path: Path | None = None) -> BrokerConfig:
    ...
    return BrokerConfig(
        ...
        broker_state_root=data.get("broker_state_root", "~/.local/share/claude/research-compute"),
        ...
        deployment_alias=data.get("deployment_alias", "research-compute-claude"),
