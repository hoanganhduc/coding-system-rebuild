...
## MANDATORY: Lean formalization gate

Before ANY Lean formalization work — whichever lane starts it (`lean4:*`
plugin skills, the Lean Formalization Team template, the formal-lane runbook,
or ad-hoc proving) — run `lean-research-library search` for each target
statement. Precedence is normative: mathlib hit → use mathlib; else personal
library (HoangMathLib) → use it; else formalize new. After a result is
accepted, run `lean-research-library intake` and ASK before any staging;
library writes and anything outward-facing (repos, pushes, Zenodo) are always
user-gated.
