...
TDD_MODE=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode 2>/dev/null || echo "false")
```

When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@$HOME/.codex/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
AI_PHASE_CFG=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
**Project skills:** Check .agents/skills/ or .codex/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.pattern_mapper --default true 2>/dev/null)
```
...
**Project instructions:** Read ./AGENTS.md if exists — follow project-specific guidelines
**Project skills:** Check .agents/skills/ or .codex/skills/ directory (if either exists) — read SKILL.md files, plans should account for project skill rules

${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @$HOME/.codex/get-shit-done/references/tdd.md to all eligible tasks:
...
**Project skills:** Check .agents/skills/ or .codex/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_passes --default 2)
BOUNCE_SCRIPT=$(node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_script)
```
...
```bash
node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
