...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  const os = require('os');
  const preferredGlobalSkillsBase = path.join(os.homedir(), '.codex', 'skills');
  const legacyGlobalSkillsBase = path.join(os.homedir(), '.claude', 'skills');
  ...
  for (const skillPath of skillPaths) {
    ...
    // Support global: prefix for skills installed at ~/.codex/skills/
    // with a legacy ~/.claude/skills/ fallback (#1992)
    if (skillPath.startsWith('global:')) {
      ...
      let globalSkillsBase = preferredGlobalSkillsBase;
      let globalSkillDir = path.join(globalSkillsBase, skillName);
      let globalSkillMd = path.join(globalSkillDir, 'SKILL.md');
      let globalSkillDisplay = `~/.codex/skills/${skillName}`;
      if (!fs.existsSync(globalSkillMd)) {
        const legacyGlobalSkillDir = path.join(legacyGlobalSkillsBase, skillName);
        const legacyGlobalSkillMd = path.join(legacyGlobalSkillDir, 'SKILL.md');
        if (fs.existsSync(legacyGlobalSkillMd)) {
          globalSkillsBase = legacyGlobalSkillsBase;
          globalSkillDir = legacyGlobalSkillDir;
          globalSkillMd = legacyGlobalSkillMd;
          globalSkillDisplay = `~/.claude/skills/${skillName}`;
        }
      }
      if (!fs.existsSync(globalSkillMd)) {
        process.stderr.write(`[agent-skills] WARNING: Global skill not found at "~/.codex/skills/${skillName}/SKILL.md" or "~/.claude/skills/${skillName}/SKILL.md" — skipping\n`);
        continue;
      }
      ...
      validPaths.push({ ref: `${globalSkillDir}/SKILL.md`, display: globalSkillDisplay });
      continue;
    }
