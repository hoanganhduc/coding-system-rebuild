...
| G6 | Malicious inputs/partial root effects/live VPN, FIFO and mount substitution, unrelated OpenVPN process scan, authenticated legacy-root migration, dead-fence compatibility-ledger recovery, and signed-bootstrap rescue | One owner, no hooks, no ambiguous process or mount state, exact owner-only cleanup, unchanged selectors during rescue, and empty root residue |
